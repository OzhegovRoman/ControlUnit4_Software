...
bool AdvancedRelayControl::getRelayState()
{
    bool rv = false;
    bool ok;
    cRelaysStatus status = mTDriver->relaysStatus()->getValueSync(&ok, 5);
    if (ok){
...
void TemperatureRecycleInterface::saveSSPDParams()
{
    ...
    for(auto a : *mDrivers) {
        ...
        if (driver){
            ...
            bool ok = false;
            double current = driver->current()->getValueSync(&ok, 5);
            if (ok)
                temp.first = current;
            bool isShorted = driver->status()->getValueSync(&ok, 5).stShorted;
            if (ok)
