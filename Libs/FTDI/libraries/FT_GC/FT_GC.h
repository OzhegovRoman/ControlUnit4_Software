...
public:
    ...
    FT_GEStatus BitmapLayout_H(uint8_t Linestride, uint8_t Height);
    ...
    FT_GEStatus PaletteSource(uint32_t address);
    ...
    FT_GEStatus WriteCmdfromflash(const uint8_t *Src, uint32_t NBytes);
    ...
    FT_GEStatus TransferCmdfromflash(const uint8_t *Src, uint32_t NBytes);
    ...
    // функции прокидывающие в SPI, чтобы можно было напрямую с ним работать
...
void FT_GC<FT_Trans>::ActiveInternalClock(void)
{
    ...
    {
        255,0,0,2,//GPU instruction CLEAR_COLOR_RGB - black color
...
void FT_GC<FT_Trans>::DisplayConfigExternalClock(uint8_t ResType)
{
    ...
    FT_Trans::Write16(REG_ROTATE, FT_DISPLAY_ROTATE);
}
template<class FT_Trans>
/* Apis related to graphics processor */	
...
template<class FT_Trans>
FT_GEStatus FT_GC<FT_Trans>::BitmapLayout_H(uint8_t Linestride, uint8_t Height)
{
    //  return (WriteCmd((7UL << 24) | ((Format & 0x1FL) << 19) | ((Linestride & 0x3FFL) << 9) | ((Height & 0x1FFL) << 0)));
    union {
        uint32_t UUInt32;
    ...
    A[0] = ((Linestride & 0x03)<<2) | (Height & 0x03);
    A[1] = 0;
    A[2] = 0;
    A[3] = 0x28;
    return ( WriteCmd(UUInt32) );
...
template<class FT_Trans>
FT_GEStatus FT_GC<FT_Trans>::PaletteSource(uint32_t address)
{
    return ( WriteCmd((42UL << 24) | (address & 0x3FFFFF)));
}
...
template<class FT_Trans>
/* APIS related to power modes */
void FT_GC<FT_Trans>::HostCommand(uint8_t HostCommand)
{
    /* construct host command and send to graphics controller */
    FT_Trans::HostCommand(HostCommand);//ideally sending 3 bytes is sufficient
...
template<class FT_Trans>
FT_GEStatus FT_GC<FT_Trans>::TransferCmdfromflash(const uint8_t *Src, uint32_t NBytes)
{
    uint32_t i,Count;
    //align the NBytes to multiple of 4
    NBytes = (NBytes + 3)&(~3);
...
FT_GEStatus FT_GC<FT_Trans>::WriteCmdfromflash(const uint8_t *Src, uint32_t NBytes)
{
    FT_GEStatus Status;
    if(0 == TrnsFlag)
    {
...
template<class FT_Trans>
uint8_t FT_GC<FT_Trans>::Read(uint32_t Addr)
{
    return (FT_Trans::Read(Addr));
}

//read a short from Addr location
template<class FT_Trans>
uint16_t FT_GC<FT_Trans>::Read16(uint32_t Addr)
{
    return (FT_Trans::Read16(Addr));
}

//read a word from Addr location
template<class FT_Trans>
uint32_t FT_GC<FT_Trans>::Read32(uint32_t Addr)
{
    return (FT_Trans::Read32(Addr));
}

//read N bytes from Addr location
template<class FT_Trans>
void FT_GC<FT_Trans>::Read(uint32_t Addr, uint8_t *Src, uint32_t NBytes)
{
    FT_Trans::Read(Addr, Src, NBytes);
}

template<class FT_Trans>
//write a byte to Addr location
void FT_GC<FT_Trans>::Write(uint32_t Addr, uint8_t Value8)
{
    FT_Trans::Write(Addr, Value8);
}

template<class FT_Trans>
//write a short to Addr location
void FT_GC<FT_Trans>::Write16(uint32_t Addr, uint16_t Value16)
{
    FT_Trans::Write16(Addr, Value16);
}

template<class FT_Trans>
//write a word to Addr location
void FT_GC<FT_Trans>::Write32(uint32_t Addr, uint32_t Value32)
{
    FT_Trans::Write32(Addr, Value32);
}

template<class FT_Trans>
void FT_GC<FT_Trans>::Write(uint32_t Addr, uint8_t *Src, uint32_t NBytes)
{
    FT_Trans::Write(Addr, Src, NBytes);
}
