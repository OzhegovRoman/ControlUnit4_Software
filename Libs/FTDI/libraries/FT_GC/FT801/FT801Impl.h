...
//	FT801Impl(uint8_t CSPin,uint8_t PDNPin,uint8_t IntPin);//all the three pins will be utilized
    ~FT801Impl();
    FT_Status Init(uint8_t ResType,uint16_t options1 = 0);//api to set the resolution of output display
	FT_Status Init(uint16_t hperiod,uint16_t vperiod,uint16_t hfrontporch,uint16_t hbackporch,uint16_t hpulsewidth,
                   ...
                   uint8_t polarity,uint8_t swizzle,uint8_t fps, uint16_t options1 = 0);/* Api to program the output display wrt display parameters */
	FT_Status Exit();
...
template<class FT_Trans>
FT_Status FT801Impl<FT_Trans>::Init(uint8_t ResType, uint16_t options1)
{
